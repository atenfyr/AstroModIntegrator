        ...
        public void IntegrateMods(string paksPath, string installPath) // @"C:\Users\<CLIENT USERNAME>\AppData\Local\Astro\Saved\Paks", @"C:\Program Files (x86)\Steam\steamapps\common\ASTRONEER\Astro\Content\Paks"
        {
            ...
            using (FileStream f = new FileStream(realPakPath, FileMode.Open, FileAccess.Read))
            {
                ...
                if (ourExtractor != null)
                {
                    ...
                    // Patch levels for biome placement modifiers and missions
                    if (biomePlacementModifiers.Count > 0 || newTrailheads.Count > 0)
                    {
                        foreach (string path in DefaultMapPaths)
                        {
                            ...
                            if (mapPathData1 != null) IntegratorUtils.SplitExportFiles(bpmBaker.Bake(biomePlacementModifiers, newTrailheads.ToArray(), IntegratorUtils.Concatenate(mapPathData1, mapPathData2)), path, CreatedPakData);
                        }
                    }

                    // Patch levels for persistent actors
